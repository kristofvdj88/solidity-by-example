...
contract Ballot {
    ...
    // @notice Function that returns the proposal count.
            ...
            returns (uint[] memory)
    {
        ...
        uint[] memory trimmedResult = new uint[](length);
        for (uint j = 0; j < trimmedResult.length; j++) {
            trimmedResult[j] = winningProposals_[j];
        }
            ...
            returns (bytes32[] memory)
    {
        ...
        for (uint256 i = 0; i < winningProposals.length; i++) {
            winnerNames_[i] = proposals[winningProposals[i]].name;
        }
        return winnerNames_;
    }
}